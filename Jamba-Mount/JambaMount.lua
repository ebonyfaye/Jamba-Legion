...
function AJM:UNIT_SPELLCAST_START(event, unitID, spell, rank, lineID, spellID, ...  )
	...
	if unitID == "player" then
		for i = 1, C_MountJournal.GetNumMounts() do
			...
			---AJM:Print("Test", id, spellID)
			if name == spell then
				local name = GetSpellInfo(id)
				AJM:Print("SendtoTeam", "name", name, "id", id, i)
				AJM:JambaSendCommandToTeam( AJM.COMMAND_MOUNT_ME, name, id )
...
function AJM:UNIT_AURA(event, unitID, ... )
	...
	if unitID ~= "player" or AJM.isMounted == nil then
        return
    end
	--AJM:Print("CheckingForMount", unitID, AJM.isMounted)
...
function AJM:TeamMount(characterName, name, id)
	AJM:Print("testTeamMount", characterName, name, id )
	...
	--Do i have the same mount as master?
		for i = 1, C_MountJournal.GetNumMounts() do
			...
			if creatureName == name then
				AJM:Print(" I have this Mount!", name, i)	
			...
			if i == C_MountJournal.GetNumMounts() then
				name = "Random"
			end
		end
	...
	-- I don't have the mount cast random!
